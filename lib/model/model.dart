...
class Model {
  static Future<List<PassInfo>> extractPkpasses() async {
  ...
  static Future<List<PassInfo>> openPkpasses({Directory directory}) async {
    ...
    for (var element in a) {
      try {
        ...
        print(e);
        toast(parent.path.substring(
                parent.path.lastIndexOf("/") + 1, parent.path.length) +
            " could not be readed");
      }
    }
    ...
    for (var e in passInfoList) {
      if (e.relevantDate != null &&
        ...
        try {
          SharedPreferences prefs = await SharedPreferences.getInstance();
          List<String> _settings = prefs.getStringList('settings');
          hideNotifications = _settings[1] == 'true';
        } catch (_) {
  ...
  static deletePkpass(String appPath, String passPath,
      {bool permanently = false}) {
    File(appPath).deleteSync(recursive: true);
    if (permanently)
      try {
        File(passPath).deleteSync();
      } catch (e) {
        print(e);
      }
  }

  static createPkpass(PassInfo passInfo) async {
    Directory externalDir = await getExternalStorageDirectory();
    var dir =
        Directory(externalDir.path + "/" + DateTime.now().toIso8601String());
    if (!await dir.exists()) {
      dir = await dir.create();
    }
    File file = File(dir.path + "/file.path");
    file.createSync();
    file.writeAsString(dir.path);
    File jsonFile = File(dir.path + "/pass.json");
    jsonFile.createSync();
    jsonFile.writeAsString(jsonEncode(passInfo));
  }
}
